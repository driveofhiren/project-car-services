...
--
-- Host: 127.0.0.1
-- Generation Time: Apr 18, 2023 at 07:52 AM
...
-- Procedures
--
CREATE DEFINER=`root`@`localhost` PROCEDURE `create_car` (IN `p_car_id` INT, IN `p_customer_id` INT, IN `p_color` VARCHAR(20), IN `p_model` VARCHAR(50), IN `p_fuel_type` VARCHAR(20), IN `p_mileage` INT, IN `p_power` INT, IN `p_car_number` VARCHAR(20))   BEGIN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `get_cars_by_customer` (IN `customer_name_in` VARCHAR(255))   BEGIN
DECLARE v_customer_id INT;
-- Get the customer ID for the given customer name
SELECT customer_id INTO v_customer_id
FROM customer
WHERE customer_name = customer_name_in;
-- Open a cursor to select all cars owned by the customer
SELECT * FROM car
WHERE customer_id = v_customer_id;
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `login_user` (IN `p_username` VARCHAR(50), IN `p_password` VARCHAR(50))   BEGIN
...
ALTER TABLE `appointment`
  ...
  ADD KEY `customera_fk` (`customer_id`);
...
--
ALTER TABLE `appointment`
  ADD CONSTRAINT `customera_fk` FOREIGN KEY (`customer_id`) REFERENCES `customer` (`customer_id`) ON DELETE CASCADE ON UPDATE NO ACTION;
...
--
ALTER TABLE `service`
  ADD CONSTRAINT `customer_fk` FOREIGN KEY (`customer_id`) REFERENCES `customer` (`customer_id`) ON DELETE CASCADE ON UPDATE NO ACTION;
COMMIT;
